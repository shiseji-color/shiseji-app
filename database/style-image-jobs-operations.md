...
1. 确认 `STYLE_IMAGE_GENERATION_ENABLED=false`，并保持应用未部署。
2. 创建并验证生产数据备份。
...
- 任务表、5 个 RPC 和私有 `style-images` 桶均存在；
- RLS 已启用并强制执行；
- `anon`、`authenticated` 无表权限和函数执行权限；
- `service_role` 仅有读取、插入、更新和 5 个 RPC 的执行权限，无删除权限；
- 5 个 RPC 都是 `SECURITY DEFINER` 且固定空 `search_path`；
...
回滚脚本会先检查任务表和私有桶。只要发现任何任务行或存储对象，就会抛出异常并让整个事务回滚，不会删除数据。检查通过后，它会移除新桶、新字段和 5 个新 RPC，并恢复 PR #19 使用的 3 个旧 RPC 与约束。
