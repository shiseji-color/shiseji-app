...
import { isSpecificAnalysisFailureCode } from './analysis-error.js';
...
export async function claimStyleImageJob(codeHash, requestId, kind, ownerId, retry = false) {
...
export async function saveStyleImageSource(
  codeHash,
  requestId,
  kind,
  ownerId,
  ...
  return callActivationRpc('save_style_image_source', codeHash, {
    p_request_id: requestId,
    p_kind: kind,
    p_owner_id: ownerId,
...
export async function saveStyleImageProviderResult(
  codeHash,
  requestId,
  kind,
  ownerId,
  ...
  return callActivationRpc('save_style_image_provider_result', codeHash, {
    p_request_id: requestId,
    p_kind: kind,
    p_owner_id: ownerId,
...
export async function saveStyleImageProviderTask(
  codeHash,
  requestId,
  kind,
  ownerId,
  ...
  return callActivationRpc('save_style_image_provider_task', codeHash, {
    p_request_id: requestId,
    p_kind: kind,
    p_owner_id: ownerId,
...
export async function beginStyleImageProviderSubmission(codeHash, requestId, kind, ownerId) {
  return callActivationRpc('begin_style_image_provider_submission', codeHash, {
    p_request_id: requestId,
    p_kind: kind,
    p_owner_id: ownerId,
  });
...
export async function completeStyleImageJob(
  codeHash,
  requestId,
  kind,
  ownerId,
...
export async function failStyleImageJob(
  codeHash,
  requestId,
  kind,
  ownerId,
...
export async function failAnalysisJob(claims, reason) {
  if (!isSpecificAnalysisFailureCode(reason)) {
    throw new Error('Analysis failure diagnostic is missing');
  }
  const result = await callActivationRpc('fail_analysis_job', claims.codeHash, {
    p_request_id: claims.requestId, p_task_id: claims.taskId,
    p_owner_id: claims.ownerId, p_reason: reason,
  });
  if (result !== true) throw new Error('Analysis failure state was not persisted');
  return true;
}
