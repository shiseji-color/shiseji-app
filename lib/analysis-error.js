const SAFE_ANALYSIS_FAILURE_CODES = new Set([
  ...
  'background_payload_invalid',
  'background_handler_failed',
  'background_worker_failed',
  'analysis_handler_invoke_failed',
  'analysis_handler_reported_failure',
  'analysis_handler_response_invalid',
...
export async function runModelCall(operation) {
  try {
    ...
    const failureCode = safelyClassifyAnalysisFailure(error, 'model_request_failed');
    throw analysisFailureError(
      failureCode === 'analysis_failed' ? 'model_request_failed' : failureCode,
    );
  }
}

export function preserveAnalysisFailure(error, fallbackCode) {
  const classified = safelyClassifyAnalysisFailure(error, fallbackCode);
  return analysisFailureError(classified === 'analysis_failed' ? fallbackCode : classified);
}

export function safelyClassifyAnalysisFailure(error, fallbackCode = 'analysis_failed') {
  try {
    return classifyAnalysisFailure(error);
  } catch {
    return SAFE_ANALYSIS_FAILURE_CODES.has(fallbackCode)
      ? fallbackCode
      : 'analysis_failed';
  }
}
...
export function isSpecificAnalysisFailureCode(code) {
  return SAFE_ANALYSIS_FAILURE_CODES.has(code) && code !== 'analysis_failed';
}
