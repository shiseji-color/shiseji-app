...
export function extractGeneratedImageUrl(payload) {
  ...
  const image = Array.isArray(content)
    ...
    : null;
  const resultUrl = Array.isArray(payload?.output?.results)
    ? payload.output.results.find((item) => typeof item?.url === 'string')?.url
    : null;
  const url = image || resultUrl;
...
export function extractStyleImageTaskId(payload) {
  const taskId = payload?.output?.task_id;
  if (typeof taskId !== 'string' || !/^[a-zA-Z0-9_-]{8,200}$/.test(taskId)) {
    throw new Error('Image model returned no valid task ID');
  }
  return taskId;
}

export function normalizeStyleImageTaskStatus(payload) {
  const status = String(payload?.output?.task_status || '').toLowerCase();
