...
export const handler = async (event, context) => {
    ...
    if (event.httpMethod !== 'POST') {
        return { statusCode: 405, body: JSON.stringify({ error: 'Error 405: Method Not Allowed' }) };
    }

    try {
        const { imageBase64 } = JSON.parse(event.body || '{}');
        if (!imageBase64) {
            return { statusCode: 400, body: JSON.stringify({ error: 'Error 400: Face Image Missing' }) };
        }

        const { API_KEY, BASE_URL } = process.env;
        if (!API_KEY || !BASE_URL) {
            return { statusCode: 500, body: JSON.stringify({ error: 'Error 500: Core Configuration Missing' }) };
        }
...
Expected JSON structure:
{
  ...
  "avoid_colors": ["String", "String"]
        ...
        if (!jsonMatch) {
            return { statusCode: 500, body: JSON.stringify({ error: 'Error 500: Data Format Collapse' }) };
        }
        ...
        console.error(error);
        return { statusCode: 500, body: JSON.stringify({ error: 'Error 500: Engine Computational Disruption' }) };
    }
};
